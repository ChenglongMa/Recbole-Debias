...
df_tr = pd.read_csv(f'{dir_name}/kuai/kuai.train.inter', sep='\t')
df_va = pd.read_csv(f'{dir_name}/kuai/kuai.valid.inter', sep='\t')
df_te = pd.read_csv(f'{dir_name}/kuai/kuai.test.inter', sep='\t')

# In[8]:


df = pd.concat([df_tr, df_va, df_te])


# In[9]:


# df['user_id:token'].nunique(), df['item_id:token'].nunique()
...
kwai_tr, kwai_va, kwai_te = split_data(df, by='user_id:token', order_by='timestamp:float', ratio=[0.8, 0.1, 0.1])

os.makedirs(f'{dir_name}/kuai_full', exist_ok=True)
os.makedirs(f'{dir_name}/kwai', exist_ok=True)

# In[23]:


df.to_csv(f'{dir_name}/kuai_full/kuai_full.inter', index=False)

# In[24]:


kwai_tr.to_csv(f'{dir_name}/kwai/kwai.train.inter', index=False)
kwai_va.to_csv(f'{dir_name}/kwai/kwai.valid.inter', index=False)
kwai_te.to_csv(f'{dir_name}/kwai/kwai.test.inter', index=False)
